...
import IconifyIcon from 'components/base/IconifyIcon';
import ProfileImage from 'assets/images/profile.png';
import paths from 'routes/paths'; // ✅ Step 2
...
const menuItems: MenuItems[] = [
  ...
  { id: 6, title: 'Logout', icon: 'ic:baseline-logout' }, // 👈 Match by title
];
      ...
      >
        ...
        <Box p={1}>
          {menuItems.map((item) => (
            <MenuItem
              key={item.id}
              onClick={item.title === 'Logout' ? handleLogout : handleProfileMenuClose} // 👈 Hook it up
              sx={{ py: 1 }}
            >
              <ListItemIcon sx={{ mr: 1, color: 'text.secondary', fontSize: 'h5.fontSize' }}>
                <IconifyIcon icon={item.icon} />
